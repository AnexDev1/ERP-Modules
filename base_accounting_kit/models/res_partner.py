...
class ResPartner(models.Model):
    ...
    def action_share_pdf(self):
        ...
        if self.customer_report_ids:
            ...
            if not self.email:
                raise ValidationError('Cannot send email: This customer has no email address configured.')
                
            body_html = Markup('<div style="color: #212529;"><p>Dear <strong> Mr/Miss. ' + self.name + \
                        '</strong> </p> <p> We have attached your ' + \
                        'payment statement. Please check </p> ' + \
                        '<p>Best regards, </p> <p> ' + self.env.user.name + '</p></div>')
                        
            # Use message_post with comment type. This still sends the email because partner_ids is set, 
            # but it renders normally in the chatter without weird white-text formatting.
            self.message_post(
                body=body_html,
