def migrate(cr, version):
    ...
    try:
        requisitions = env['store.requisition'].search([('department_id', '=', False)])
        for req in requisitions:
            ...
            if employee and hasattr(employee, 'department_id') and employee.department_id:
                cr.execute(
                    "UPDATE store_requisition SET department_id = %s WHERE id = %s",
                    (employee.department_id.id, req.id)
                )
    except Exception as e:
        pass # Ignore if any ORM failure occurs to avoid blocking upgrade
